...
Additionally, you'll need to configure the Muthur Command Core to pick up the SSL certificates. This is done by setting the following configuration for the [HTTP][HTTP] integration configuration in your `configuration.yaml`:
...
The name of the certificate file generated by Let's Encrypt. The file is used for SSL by Muthur Command apps and is recommended to keep the filename as-is (`fullchain.pem`) for compatibility.

**Note**: _The file is stored in `/ssl/`, which is the default for Muthur Command_

#### Option `lets_encrypt.keyfile`

The name of the private key file generated by Let's Encrypt. The private key file is used for SSL by Muthur Command apps and is recommended to keep the filename as-is (`privkey.pem`) for compatibility.
...
[HTTP]: https://www.muthur-command.com/integrations/http/
